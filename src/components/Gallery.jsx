...
export default function Gallery({ onSelectService }) {
  ...
  return (
    <section id="galeria" className="py-20 sm:py-28 relative bg-dark-950">
      {/* Subtle Glow */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Filter Pills */}
        <div className="flex justify-center mb-8 sm:mb-12 overflow-x-auto pb-2 scrollbar-none">
        ...
        {/* Instagram Grid (2 columns on mobile, 4 on desktop for authentic Instagram feel) */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
          {filteredItems.map((item) => (
            <div
              key={item.id}
              onClick={() => setSelectedImage(item)}
              className="relative group rounded-xl sm:rounded-2xl overflow-hidden cursor-pointer bg-dark-900 border border-neutral-800/80 hover:border-gold-400/50 transition-all duration-500 shadow-lg hover:shadow-gold-sm aspect-[4/5]"
            >
              {/* Photo */}
              <img
                src={item.image}
              ...
              <div className="absolute top-2.5 left-2.5 right-2.5 sm:top-4 sm:left-4 sm:right-4 flex items-center justify-between pointer-events-none">
                <span className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-dark-950/80 backdrop-blur-md text-gold-300 text-[9px] sm:text-[11px] font-medium border border-gold-400/30 truncate max-w-[110px]">
                  {item.tag}
                </span>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-5">
                <h3 className="font-serif text-xs sm:text-base font-bold text-neutral-100 mb-0.5 sm:mb-1 drop-shadow-md truncate">
                  {item.title}
                </h3>
                <p className="text-[10px] sm:text-xs text-neutral-300/90 line-clamp-1 sm:line-clamp-2 mb-2 hidden sm:block">
                  {item.description}
                </p>

                <div className="flex items-center justify-between text-[10px] sm:text-xs text-neutral-400 pt-1.5 sm:pt-2 border-t border-neutral-700/40">
        ...
        <div className="mt-10 sm:mt-14 p-5 sm:p-8 rounded-2xl bg-gradient-to-r from-dark-900 via-dark-850 to-dark-900 border border-gold-400/25 flex flex-col sm:flex-row items-center justify-between gap-5 sm:gap-6 shadow-xl text-center sm:text-left">
          <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-tr from-amber-600 via-rose-600 to-purple-600 p-[2px] shadow-lg shrink-0">
          ...
          <a
            ...
            className="shimmer-btn w-full sm:w-auto px-6 py-3 rounded-full bg-gold-metallic text-dark-950 font-bold text-xs sm:text-sm shadow-gold-sm hover:scale-105 active:scale-95 transition-all shrink-0 flex items-center justify-center gap-2"
          >
      ...
      {selectedImage && (
        ...
        >
          <div 
            className="relative max-w-2xl w-full max-h-[92vh] overflow-y-auto bg-dark-900 rounded-2xl border border-gold-400/40 shadow-2xl"
