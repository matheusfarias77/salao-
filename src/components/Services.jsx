...
export default function Services({ onSelectService }) {
  ...
  return (
    <section id="servicos" className="py-24 sm:py-32 relative bg-dark-900 border-t border-neutral-900/80 overflow-hidden w-full max-w-full">
      {/* Subtle Ambient Glow */}
      <div className="absolute top-1/3 left-0 w-full max-w-[350px] h-[350px] bg-gold-500/5 blur-[160px] rounded-full pointer-events-none"></div>
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex justify-center mb-12 sm:mb-16">
          <div className="inline-flex p-1 rounded-full bg-dark-950/90 border border-[#D4AF37]/25 shadow-md max-w-full overflow-x-auto scrollbar-none">
            {serviceCategories.map((cat) => (
              <button
                ...
                className={`px-4 sm:px-6 py-2 sm:py-2.5 rounded-full text-xs sm:text-sm font-medium transition-all duration-300 whitespace-nowrap ${
                  activeTab === cat.id
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-start">
              ...
              return (
                <div
                  ...
                  className={`p-5 sm:p-6 rounded-2xl transition-all duration-300 cursor-pointer border ${
                    isSelected
                      ? 'bg-dark-850/90 border-[#D4AF37]/50 shadow-gold-sm -translate-y-0.5'
                      : 'bg-dark-950/70 border-neutral-800/70 hover:border-[#D4AF37]/30 hover:bg-dark-850/50'
                  }`}
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2.5">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      <h3 className="font-serif text-lg sm:text-xl font-medium text-neutral-100">
                        {service.title}
                      </h3>
                      {service.badge && (
                        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-medium uppercase tracking-wider bg-gold-500/10 text-gold-300 border border-[#D4AF37]/25">
                          {service.badge}
                        </span>
                      )}
                    </div>
                    <span className="inline-flex items-center gap-1.5 text-xs text-neutral-400 font-light shrink-0">
                      <Clock className="w-3.5 h-3.5 text-gold-400/90" />
                      {service.duration}
                    </span>
                  </div>

                  <p className="text-neutral-300/80 font-light text-xs sm:text-sm leading-relaxed mb-4">
                    {service.description}
                  </p>

                  {/* Included points */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4 pt-3 border-t border-neutral-800/60">
                    {service.includes.map((inc, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs text-neutral-400 font-light">
                        <Check className="w-3.5 h-3.5 text-gold-400/90 shrink-0" />
                  ...
                  <div className="pt-2 flex items-center justify-end">
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gold-metallic hover:bg-gold-gradient-hover text-dark-950 font-semibold text-xs transition-all hover:scale-105 active:scale-95"
                    >
          ...
          <div className="lg:col-span-5 hidden lg:block sticky top-28">
            <div className="relative rounded-3xl overflow-hidden border border-[#D4AF37]/30 bg-dark-950 shadow-xl">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-dark-950 via-dark-950/90 to-transparent backdrop-blur-[2px]">
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-gold-metallic text-dark-950 font-semibold text-xs shadow-sm hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                >
